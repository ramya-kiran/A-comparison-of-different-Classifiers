...
nb_model <- naiveBayes(as.factor(class.train) ~ .,data = input_data)
results <-predict(nb_model,newdata = test_data)
as.numeric(as.matrix(results))
print("The error rate is as follows")
print(sum(results != class.test)/length(class.test))
print("The confusion matrix is as follows")
print(table(results,class.test))
