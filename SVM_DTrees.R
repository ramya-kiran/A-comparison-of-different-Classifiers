...
fit = rpart(class.train ~ ., data=input_data, method="class",minbucket=1,cp=0)
pred = predict(fit, test_data, type='vector')
printcp(fit)
print("The generalization error for Decision Trees is as follows")
print(sum(pred != class.test)/length(class.test))
print("The confusion matrix is as follows")
print(table(pred,class.test))

# Classification using SVM
input_data = input_data[,-3]
model = svm(class.train ~., input_data)
summary(model)

pred <- predict(model,test_data)
pred = floor(pred)
print("The generalization error for SVM is as follows")
print(sum(pred != class.test)/length(class.test))
print("The confusion matrix is as follows")
print(table(pred,class.test))
